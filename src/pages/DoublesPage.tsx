  ...
  return (
    <div className="space-y-4 pb-2">
      <div>
        ...
        <p className="mt-1 text-body text-muted">{copy.formats.doubles.subtitle}</p>
        ...
        <div className="space-y-2">
          {DOUBLES_TERMINOLOGY.map((t) => (
            <article key={t.title} className="app-panel p-4">
              <h2 className="text-body font-semibold text-accent">{t.title}</h2>
              <p className="mt-2 text-caption leading-relaxed text-muted">{t.body}</p>
            </article>
          ))}
        </div>
      )}
